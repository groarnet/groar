...
//	private static $__secret_key = $globals['Amazon_secret_key'];
	...
	public static function put($file, $type, $name = false) {
		...
		if ($response) {
			// syslog(LOG_NOTICE, "groar, uploaded $uri to S3");
			$lastHTTPCode = 200;
			return true;
		}
		syslog(LOG_NOTICE, "groar, failed to upload $uri to S3");
		$lastHTTPCode = 0;
		return false;
	}
	...
	public static function get($file, $type, $output = false) {
		...
		// syslog(LOG_NOTICE, "groar, failed to get $uri from S3 to $output code: " . S3::$lastHTTPCode);
		@unlink($output);
		return false;
	}
