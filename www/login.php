...
function do_login() {
	...
	// Show groar intro only if first try and the there were not previous logins
	if (! $globals['mobile'] && $previous_login_failed < 3 && empty($_POST["processlogin"]) && empty($_COOKIE['u'])) {
		...
		echo '<h3>'._('¿Todavía no eres usuario de groar?').'</h3>'."\n";
		...
		echo '<p>'._('Una vez registrado puedes enviar las historias que consideres interesantes para la comunidad. Si tienes algún tipo de duda sobre que tipo de historias puedes enviar revisa nuestras <a href="faq-es.php">preguntas frecuentes sobre groar</a>').'.</p>'."\n";
		echo '</li>'."\n";
		echo '<li>'."\n";
		echo '<strong>'._('Escribir comentarios').'</strong><br />'."\n";
		echo '<p>'._('Puedes escribir tu opinión sobre las historias enviadas a groar mediante comentarios de texto. También puedes votar positivamente aquellos comentarios ingeniosos, divertidos o interesantes y negativamente aquellos que consideres inoportunos').'.</p>'."\n";
		echo '</li>'."\n";
		echo '<li>'."\n";
		echo '<strong>'._('Perfil de usuario').'</strong><br />'."\n";
		echo '<p>'._('Toda tu información como usuario está disponible desde la página de tu perfil. También puedes subir una imagen que representará a tu usuario en groar. Incluso es posible compartir los ingresos publicitarios de groar, solo tienes que introducir el código de tu cuenta Google Adsense desde tu perfil').'.</p>'."\n";
		echo '</li>'."\n";
		echo '<li>'."\n";
		echo '<strong>'._('Chatear en tiempo real desde la fisgona').'</strong><br />'."\n";
		echo '<p>'._('Gracias a la <a href="sneak.php">fisgona</a> puedes ver en tiempo real toda la actividad de groar. Además como usuario registrado podrás chatear con mucha más gente de la comunidad groar').'</p>'."\n";
		echo '</li>'."\n";
		echo '</ul>'."\n";
