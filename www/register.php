...
function check_user_fields() {
	...
	if (empty($globals['skip_ip_register'])) {
		...
		if($registered > 0) {
			syslog(LOG_NOTICE, "groar, register not accepted by IP address ($_POST[username]) $user_ip");
		...
		if($registered > 0) {
			syslog(LOG_NOTICE, "groar, register not accepted by IP class ($_POST[username]) $ip_class");
