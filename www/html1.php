...
if (! $globals['bot'] && ($globals['allow_partial'] || preg_match('/groar/i', $_SERVER['HTTP_USER_AGENT']))) {
...
function do_header($title, $id='home', $options = false) {
	...
	if ($this_site->sub) {
		$this_site->url = $this_site->base_url.'gr/'.$this_site->name;
	} else {
	...
	if (! is_array($options)) {
		...
		$right_options = array();
		$right_options[] = new MenuOption(_('gr/'), $globals['base_url_general'].'subs', $id, _('sub groars'));
